...
class ImageModifier:
    ...
    def color_image_callback(self, data):
        try:
            ...
            # Generate translucent red blobs and blend them
            modified_image = self.add_red_blobs(cv_image)

            # Copy header from input to output to preserve metadata
            output_msg = self.bridge.cv2_to_imgmsg(modified_image, "bgr8")
            output_msg.header = data.header  # This ensures the header is copied exactly
    ...
    def add_red_blobs(self, image):
        ...
        for _ in range(num_blobs):
            ...
            if blob_type == 'circle_pair':
                ...
                # Draw two overlapping red circles
                cv2.circle(blob_image, (x1, y1), radius1, (0, 0, 255), -1)
                cv2.circle(blob_image, (x2, y2), radius2, (0, 0, 255), -1)

            elif blob_type == 'ellipse_pair':
                # Generate two red ellipses
                for _ in range(2):  # Draw two ellipses
                    ...
                    cv2.ellipse(blob_image, center, axes, angle, 0, 360, (0, 0, 255), -1)
