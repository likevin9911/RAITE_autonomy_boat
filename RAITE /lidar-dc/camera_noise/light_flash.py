...
class ImageFlashInjector:
    ...
    def image_callback(self, data):
        try:
            ...
            # Inject flashes of light with a sky-blue tint
            flashed_image = self.add_sky_blue_flashes(cv_image)
    ...
    def add_sky_blue_flashes(self, image):
        # Number of flashes per frame
        num_flashes = np.random.randint(1, 5)  # Random number of flashes between 1 and 2

        for _ in range(num_flashes):
            # Select random location for the flash
            x, y = np.random.randint(0, image.shape[1]), np.random.randint(0, image.shape[0])
            radius = np.random.randint(50, 150)  # Radius of the flash

            # Create a mask to apply the sky-blue flash
            mask = np.zeros_like(image)
            cv2.circle(mask, (x, y), radius, (235, 206, 135), -1)  # Sky blue tint in BGR format
