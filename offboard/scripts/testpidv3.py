...
class BoatPIDController:
    ...
    def compute_angular_difference(self, future_poses):
        """Compute the angular difference based on the average yaw of the first 4 poses and the boat's current yaw."""
        ...
        # Calculate the average yaw using trigonometric averaging
        avg_yaw = math.atan2(sum(math.sin(y) for y in yaws) / 4,
                             sum(math.cos(y) for y in yaws) / 4)

        # Calculate angular difference between average yaw and boat's current yaw
        angular_difference = avg_yaw - self.yaw
        angular_difference = (angular_difference + math.pi) % (2 * math.pi) - math.pi  # Wrap to [-?, ?]
    ...
    def control_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if goal_behind:
                # Adjust angular difference for reverse motion
                angular_difference = abs(angular_difference)
                if angular_difference > 0 and future_poses[2].position.y < current_position.y:
                ...
                adjusted_linear = -1.0 * self.adjust_linear_velocity_based_on_angular(
                    angular_difference, max_linear_speed
                )
            else:
