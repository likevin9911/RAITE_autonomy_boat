...
class BoatPIDController:
    ...
    def imu_callback(self, msg):
        """Callback for zeroed IMU data."""
        self.angular_velocity_imu = msg.angular_velocity.z
        self.acceleration_imu = np.array([
    ...
    def odom_callback(self, msg):
        ...
        # Calculate linear velocity from odometry
        self.odom_velocity = math.sqrt(msg.twist.twist.linear.x ** 2 +
    ...
    def is_goal_behind(self, current_position, future_poses):
        """Determine if the goal is behind the boat."""
        # Calculate the average position of the first three poses
        avg_x = sum([pose.position.x for pose in future_poses[:3]]) / 3.0
        avg_y = sum([pose.position.y for pose in future_poses[:3]]) / 3.0

        # Vector from boat to average pose
        ...
        # If dot product is negative, the goal is behind
        return dot_product < 0

    def compute_angular_from_pose(self, current_position, future_poses):
        """Compute angular error based on the average of poses 3-5."""
        if len(future_poses) < 5:
            rospy.logwarn("Insufficient future poses to calculate angular difference.")
            return 0.0, 0.0  # Return angular error and delta_y

        # Calculate the average position of poses 3, 4, and 5
        avg_x = sum([pose.position.x for pose in future_poses[2:5]]) / 3.0
        avg_y = sum([pose.position.y for pose in future_poses[2:5]]) / 3.0

        delta_x = avg_x - current_position.x
        delta_y = avg_y - current_position.y
        ...
        # Apply angular deadband to ignore very small angular errors
        if abs(angular_error) < self.angular_deadband:
            angular_error = 0.0

        rospy.loginfo(f"Angular Difference (Avg Poses 3-5): {angular_error} radians")
        return angular_error, delta_y  # Also return delta_y

    def adjust_linear_velocity_based_on_angular(self, angular_vel, max_linear):
        """Adjust linear velocity based on angular velocity magnitude."""
        abs_angular = abs(angular_vel)

        if abs_angular >= math.pi / 2:
            # Stop linear velocity if angular error is 90 degrees or more
            return 0.0
        else:
            # Scale linear velocity based on angular error
            scale = (math.pi / 2 - abs_angular) / (math.pi / 2)
            return max_linear * scale
    ...
    def control_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            goal_behind = self.is_goal_behind(current_position, future_poses)

            # Compute angular error and delta_y
            angular_error, delta_y = self.compute_angular_from_pose(current_position, future_poses)
            angular_error = (angular_error + math.pi) % (2 * math.pi) - math.pi

            if goal_behind:
                # Adjust angular error for reverse motion
                if angular_error < 0:
                ...
                # If the goal is in the left-back quadrant, angular velocity should be negative
                if angular_error > 0 and delta_y < 0:
                    angular_error *= -1

                # Limit linear velocity to +-0.2
                max_linear_speed = self.max_linear_reverse_cmd
                # Reverse direction
                linear_direction = -1.0
                adjusted_linear = linear_direction * self.adjust_linear_velocity_based_on_angular(angular_error, max_linear_speed)
            else:
                # Forward motion logic
                target_pose = self.teb_poses[2].position
                linear_error = math.sqrt((target_pose.x - current_position.x) ** 2 + (target_pose.y - current_position.y) ** 2)

                if angular_error > 0:
                    rospy.loginfo("Turning Left (Positive Angular)")
                else:
                    rospy.loginfo("Turning Right (Negative Angular)")
                ...
                adjusted_linear = self.adjust_linear_velocity_based_on_angular(angular_error, self.max_linear_cmd)

            # Use the IMU angular velocity for feedback
            adjusted_angular = angular_error - self.angular_velocity_imu

            # PID for angular velocity
            adjusted_angular, self.integral_angular = self.compute_pid(
                adjusted_angular, self.prev_error_angular, self.integral_angular,
            ...
            rospy.loginfo(f"Angular Difference: {angular_error} radians")
    ...
    def stop_reverse_condition(self):
        """Check if the boat should stop reversing based on IMU and odometry thresholds."""
