...
def pointcloud_callback(pointcloud_msg):
    ...
    seg.set_distance_threshold(0.08)  # Adjust the threshold according to your environment

    inliers, coefficients = seg.segment()

    if len(inliers) == 0:
        rospy.logwarn("Could not estimate a planar model for the given dataset.")
        return

    # Extract ground plane points
    ground_plane = pcl_data.extract(inliers, negative=False)

    # Extract non-ground points (objects)
    objects = pcl_data.extract(inliers, negative=True)

    # Visualize the ground plane points
    ground_plane_msg = create_point_cloud_message(ground_plane, pointcloud_msg.header)
    ground_plane_pub.publish(ground_plane_msg)

    # Visualize the non-ground (object) points
    objects_msg = create_point_cloud_message(objects, pointcloud_msg.header)
    objects_pub.publish(objects_msg)

    # Filter points by minimum Z height
    passthrough = objects.make_passthrough_filter()
    passthrough.set_filter_field_name("z")
    passthrough.set_filter_limits(MIN_Z_HEIGHT, np.max(objects.to_array()[:, 2]))
...
def create_point_cloud_message(pcl_data, header):
    """Helper function to create a PointCloud2 message from PCL data."""
    points = pcl_data.to_array()
    cloud_msg = pc2.create_cloud_xyz32(header, points[:, 0:3])
    return cloud_msg
...
if __name__ == "__main__":
    ...
    # Publishers for RANSAC visualization
    ground_plane_pub = rospy.Publisher("/ground_plane", PointCloud2, queue_size=10)
    objects_pub = rospy.Publisher("/objects", PointCloud2, queue_size=10)
