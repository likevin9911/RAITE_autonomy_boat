...
# Global variables
current_gps = (37.8715, -122.2730)  # Static GPS coordinates for testing
bbox_gps_storage = {}

def gps_callback(gps_msg):
    global current_gps
    # Normally this would update with real GPS data, but we're using static coordinates for testing
    current_gps = (37.8715, -122.2730)  # Berkeley, CA
...
def simulate_gps():
    ...
    while not rospy.is_shutdown():
        # Simulate movement by incrementing the latitude and longitude slightly
        latitude += 0.00001  # Incrementing to simulate movement
        longitude += 0.00001
        current_gps = (latitude, longitude)
        rospy.sleep(1)  # Update every second

def apply_mock_transform(x, y, z):
    """
    Apply a mock transformation from the LiDAR frame to a hypothetical base_link frame.
    Adjust the values below based on your setup.
    """
    # Example static transformation (translation + rotation)
    # Assuming a simple translation with no rotation
    # Adjust these values based on your actual setup
    translation = np.array([1.0, 0.0, 0.0])  # Example translation vector (e.g., 1 meter forward)
...
def pointcloud_callback(pointcloud_msg):
    ...
    if len(front_cloud) > 0:
        ...
        for indices in cluster_indices:
            ...
            # Directly use the LiDAR frame for GPS conversion
            # Assuming the LiDAR frame is already closely aligned with the GPS frame
            # Otherwise, you may apply a mock transform using the apply_mock_transform function
            base_link_x, base_link_y, base_link_z = apply_mock_transform(center_x, center_y, center_z)
            ...
            marker.scale.x = max_point[0] - min_point[0] + 0.2
            marker.scale.y = max_point[1] - min_point[1] + 0.2
            marker.scale.z = max_point[2] - min_point[2] + 0.2
...
if __name__ == "__main__":
    ...
    # Initialize TF listener (optional, only if you use actual transforms)
    # tf_buffer = tf2_ros.Buffer()
    # listener = tf2_ros.TransformListener(tf_buffer)

    # Start the GPS simulation in a separate thread
    gps_thread = threading.Thread(target=simulate_gps)
    gps_thread.start()
