...
# Global variables for GPS simulation
gps_memory = []
tf_buffer = None 
...
def remember_location(latitude, longitude):
    # Store GPS coordinates in the memory
    gps_memory.append((latitude, longitude))
    rospy.loginfo(f"Remembered location: Latitude = {latitude}, Longitude = {longitude}") 
...
def simulate_gps(center_x, center_y):
    try:
        ...
        # Store the GPS coordinates in memory comment this for csv
        remember_location(latitude, longitude) 
